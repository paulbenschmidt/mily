...
export function PrivacyIcon({ privacyLevel }: { privacyLevel: string }) {
  const item = PRIVACY_CONFIG[privacyLevel];
  if (!item) return null;

  return (
    <div className="w-6 h-6 rounded-full bg-secondary-100 flex items-center justify-center" aria-label={item.label} role="img">
