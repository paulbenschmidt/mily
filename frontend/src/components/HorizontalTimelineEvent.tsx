...
interface HorizontalTimelineEventProps {
  event: TimelineEventType;
  isLast?: boolean;
  onEditEvent?: (event: TimelineEventType) => void;
}

export function HorizontalTimelineEvent({ event, isLast = false, onEditEvent }: HorizontalTimelineEventProps) {
  ...
  const getLineLeftOffset = (category: string) => {
    switch (category) {
      case "major":
        return 'left-7'; // larger dot needs more space
      case "minor":
        return 'left-6'; // medium dot
      case "memory":
        return 'left-6'; // smaller dot
      default:
        return 'left-6';
    }
  };
  ...
  return (
    <div
      className="relative flex flex-col gap-6 pr-8 flex-shrink-0"
    ...
    >
      ...
      <div className="flex flex-row items-center relative pl-1">
        {/* Create a fixed-height container for the dot to ensure consistent positioning */}
        ...
        {!isLast && <div className={`absolute ${getLineLeftOffset(event.category)} top-1/2 transform -translate-y-[0.5px] h-px bg-secondary-300 w-full`} />}
      </div>
      ...
      <div className="flex-1 min-h-0">
        <div className="flex flex-col gap-2 mb-2">
          ...
          <Badge className={
            event.category === 'major' ? 'border-2 border-secondary-500 bg-secondary-200 w-fit' :
            event.category === 'minor' ? 'border-2 border-secondary-400 bg-secondary-100 w-fit' :
            'border-2 border-secondary-300 bg-transparent w-fit'
          }>
            {event.category}
          </Badge>
        </div>
